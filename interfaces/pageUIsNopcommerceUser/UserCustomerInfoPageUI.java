package pageUIsNopcommerceUser;

public class UserCustomerInfoPageUI {
	public static final String LOGIN_BUTTON = "class=login-button";
	public static final String EMAIL_TEXTBOX = "id=Email";
	public static final String PASSWORD_TEXTBOX = "id=Password";
	public static final String EMAIL_ERROR_MESSAGE = "css=span[id='Email-error']";
	public static final String LOGIN_UNSUCCESS_ERROR_MESSAGE = "xpath=//div[contains(@class,'message-error')]";
	public static final String CUSTOMER_INFO_PAGE_TITTLE = "xpath=//h1[text()='My account - Customer info']";
}
